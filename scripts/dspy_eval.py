...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evalua un programa DSPy guardado.")
    ...
    artifact_path = args.artifact or Path("artifacts/dspy") / f"{args.task}.json"
    ...
    program = build_programs()[args.task]
    ...
    for example in examples:
        prediction = program(**{key: getattr(example, key) for key in config["input_keys"]})
        total += metric(example, prediction)

    score = total / len(examples) if examples else 0.0
    print(f"Task={args.task} score={score:.3f} examples={len(examples)} artifact={artifact_path}")
